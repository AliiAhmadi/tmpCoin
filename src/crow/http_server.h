...
class Server {
public:
  Server(Handler *handler, const tcp::endpoint &endpoint,
         std::string server_name = std::string("Crow/") + VERSION,
         std::tuple<Middlewares...> *middlewares = nullptr,
         uint16_t concurrency = 1, uint8_t timeout = 5,
         typename Adaptor::context *adaptor_ctx = nullptr)
      : acceptor_(io_context_, endpoint), signals_(io_context_),
        tick_timer_(io_context_), handler_(handler), concurrency_(concurrency),
        timeout_(timeout), server_name_(server_name),
        task_queue_length_pool_(concurrency_ - 1), middlewares_(middlewares),
  ...
  void do_accept() {
    if (!shutting_down_) {
      ...
      acceptor_.async_accept(
          p->socket(), [this, p, &ic, context_idx](error_code ec) {
            if (!ec) {
