...
public:
  ...
  void handle_upgrade(const request &req, response &,
    ...
    new crow::websocket::Connection<SSLAdaptor, App>(
        req, std::move(adaptor), app_, max_payload_, subprotocols_,
        open_handler_, message_handler_, close_handler_, error_handler_,
        accept_handler_, mirror_protocols_);
  }
  ...
  template <typename Func> self_t &onaccept(Func f) {
    accept_handler_ = f;
    return *this;
  }
...
private:
  ...
  void debug_node_print(const Node &node, int level) {
    if (node.param != ParamType::MAX) {
      switch (node.param) {
      case ParamType::INT:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<int>";
        break;
      case ParamType::UINT:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<uint>";
        break;
      case ParamType::DOUBLE:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<double>";
        break;
      case ParamType::STRING:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<string>";
        break;
      case ParamType::PATH:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<path>";
        break;
      default:
        CROW_LOG_DEBUG << std::string(3 * level, ' ') << "└➝ " << "<ERROR>";
        break;
      }
    } else
...
public:
  ...
  find(const std::string &req_url, const Node &node, unsigned pos = 0,
       routing_params *params = nullptr,
       std::vector<uint16_t> *blueprints = nullptr) const {
  ...
  void add(const std::string &url, uint16_t rule_index,
...
public:
  ...
  void get_found_bp(std::vector<uint16_t> &bp_i,
                    std::vector<Blueprint *> &blueprints,
                    std::vector<Blueprint *> &found_bps, uint16_t index = 0) {
  ...
  /// Is used to handle errors, you insert the error code, found route, request,
  /// and response. and it'll either call the appropriate catchall route
  /// (considering the blueprint system) and send you a status string (which is
  /// mainly used for debug messages), or just set the response code to the
  /// proper error code.
  std::string get_error(unsigned short code, routing_handle_result &found,
  ...
  std::unique_ptr<routing_handle_result> handle_initial(request &req,
    ...
    std::unique_ptr<routing_handle_result> found{new routing_handle_result(
        0, std::vector<uint16_t>(), routing_params(),
