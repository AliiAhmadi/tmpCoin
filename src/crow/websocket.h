...
public:
  ...
  Connection(
      const crow::request &req, Adaptor &&adaptor, Handler *handler,
  ...
  ~Connection() noexcept override {
    ...
    while (watch.use_count() >
           2) // 1 for 'check_destroy() routine', 1 for 'this->anchor_'
    {
...
protected:
  ...
  void do_read() {
    ...
    case WebSocketReadState::Len16: {
      ...
      asio::async_read(adaptor_.socket(), asio::buffer(&remaining_length16_, 2),
                       [this](const error_code &ec, std::size_t
#ifdef CROW_ENABLE_DEBUG
                                                        bytes_transferred
#endif
    ...
    case WebSocketReadState::Len64: {
      asio::async_read(adaptor_.socket(), asio::buffer(&remaining_length_, 8),
                       [this](const error_code &ec, std::size_t
#ifdef CROW_ENABLE_DEBUG
                                                        bytes_transferred
#endif
